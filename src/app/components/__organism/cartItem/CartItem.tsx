...
//     >
//       <div
//         className={` w-[23.32%] md:w-[19.37%]  h-full  lg:h-full bg-yellow-100`}
//       >
//         {item.presignedUrl ? (
//           <Image
...
//       <div
//         className={`${
//           isCartPage && "md:h-[169px]"
//         } flex-1 h-full flex flex-col items-start gap-2 md:gap-0 md:justify-between   relative group bg-violet-400 `}
//       >
...
//       <div className="h-full font-semibold w-[22.32%] md:w-[21%] flex flex-col items-start gap-2 bg-lime-300">
...
//   show,
  ...
  return (
    ...
    >
      ...
      >
        <div className={`${isCartPage ? "lg:w-[32.65%] flex-1" : "flex-1"}   h-full flex flex-col items-start gap-2 md:gap-0 md:justify-between`}>
          <p className="text-xs md:text-sm font-semibold leading-[22px] text-[#141718]            ">
            {productName}
          </p>
          <p className="text-xs font-normal leading-[20px] text-black/60 cursor-pointer           ">
          ...
          <div className={`${isCartPage && "lg:hidden"}`}>
            <Counter
              ...
              onChange={(newQty: number) =>
                updateCartQty(_id, color, newQty)
              }
              isCartPage={isCartPage}
              show={true}
            />
      ...
      <div
        className={`${
          isCartPage && "lg:w-[49.14%] lg:flex-row lg:items-center lg:justify-between lg:text-center"
      ...
      >
        ...
        <p className={`${isCartPage ? "lg:flex hidden" : "hidden"}`}>${price.toFixed(2)}</p>
      </div>
    </div>
  );
};
